...
def get_args(known=False):
    ...
    parser.add_argument('--project', type=str, default=os.path.dirname(os.path.realpath(__file__)) + '', help='project path for saving results')
    ...
    parser.add_argument('--inter_weight', type=float, default=0, help='inter losses weighted coefficients in the loss function')
    parser.add_argument('--model_weights', type=str, default='last.pth', help='model weights')
...
def test():
    ...
    # Load Data
    with open(args.data_path + '/Flods/test.list', 'r') as f:
    ...
    for case in tqdm(image_list):
        h5f = h5py.File(args.data_path + "/LA_Data/" + case + "/mri_norm2.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]

...
class LADataset(Dataset):
    def __init__(self, image_path='', image_size=[80, 112, 112], stage='train', is_augmentation=False):
        ...
        if self.stage == 'train':
            with open(self.image_path + "/Flods/train.list", "r") as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace("\n", "") for item in self.sample_list]
        else:
            with open(self.image_path + "/Flods/test.list", "r") as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace("\n", "") for item in self.sample_list]
    ...
    def __getitem__(self, item):
        ...
        h5f = h5py.File(self.image_path + "/LA_Data/" + image_name + "/mri_norm2.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        ...
        if self.stage == 'train':
            ...
            # print("sample_shape", sample['image'].shape)
